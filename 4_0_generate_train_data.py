...
address_fields_file = "full_dataset.csv"
...
def add_individual_company_name(text, name_prob=0.05):
    if random.random() < name_prob:
        ...
        if name_type == 'individual':
            ...
            return f"{fake.company()} {text}"
    return text


null_values = ['NULL', '']
address_df = pd.read_csv(address_fields_file, dtype='string', na_filter=null_values)

train_df = address_df.sample(n=100_000).reset_index(drop=True)
...
train_df['street_name'] = train_df['street_name'].apply(
    lambda x: introduce_typo(x, typo_prob=0.05)
    )

train_df['locality_name'] = train_df['locality_name'].apply(
    lambda x: introduce_typo(x, typo_prob=0.05)
    )
...
train_df["state_random"] = train_df.apply(
    lambda row: row["state_code"] if random.random() < 0.1 else '',
    axis=1
    )
...
train_df["country"] = train_df.apply(
    lambda row: row["country"] if random.random() < 0.05 else '',
    axis=1
    )
